---
title: "Augabe 10.1"
output: html_document
---

Formeln für den MSE

#T1 Schätzer
```{r}
library(reshape2)
x <- seq(0.1,0.9,0.1)
n <- c(10,60,100)
MSE_T1_10 <- (x-x^2)/n[1]
MSE_T1_60 <- (x-x^2)/n[2]
MSE_T1_100 <- (x-x^2)/n[3]
df <- data.frame(x,MSE_T1_10,MSE_T1_60,MSE_T1_100)
df <- melt(df, id.vars="x", variable.name="N")
```
```{r}
library(ggplot2)
ggplot(df,aes(x, value)) + geom_line(aes(colour=N))
```
#T2 Schätzer
```{r}
x <- seq(0.1,0.9,0.1)
n <- c(10,60,100)
MSE_T2_10 <- (-n[1]*(x^2+x+n[1]+2)+4*x)/((n[1]+2)^2)
MSE_T2_60 <- (-n[2]*(x^2+x+n[2]+2)+4*x)/((n[2]+2)^2)
MSE_T2_100 <- (-n[3]*(x^2+x+n[3]+2)+4*x)/((n[3]+2)^2)
df_2 <- data.frame(x,MSE_T2_10,MSE_T2_60,MSE_T2_100)
df_2 <- melt(df_2, id.vars="x", variable.name="N")
```
```{r}
library(ggplot2)
ggplot(df_2,aes(x, value)) + geom_line(aes(colour=N))
```
# braucht man glaub ich nicht:
```{r}
binomial<-function(x,n){
  A<-matrix(0,n,length(x))
  for (i in 1:length(x)){
    A[,i]=rbinom(n,1,x[i])
  }
A<-data.frame(A)
colnames(A) <- x
return(A)
}
binomial(x,10)
```

